/**
 * Cloudflare Worker — 表单后端（POST /api/lead）
 *
 * 用途：接收 Minova Titan V 落地页线索，依次完成：
 *   1) 字段校验
 *   2) Turnstile 人机校验（防垃圾提交）
 *   3) 写入飞书多维表格（CRM 线索库）
 *   4) 可选：飞书群自定义机器人即时通知
 *
 * 部署后地址形如：https://<worker名>.<你的子域>.workers.dev/api/lead
 * （前端 index.html 的 LEAD_API_URL 指向这里，跨域调用，无需绑定自有域名。）
 *
 * 环境变量（Cloudflare Dashboard → Workers → 你的 Worker → Settings → Variables & Secrets）：
